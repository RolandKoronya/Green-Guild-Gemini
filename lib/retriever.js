...
function expandQuery(q) {
  ...
  for (const p of parts) {
    if (SYNONYMS[p]) {
      for (const s of SYNONYMS[p]) bag.add(normalize(s));
    }
  }
...
export function createRetriever(
  kb,
  { geminiApiKey, embedModel = "gemini-embedding-001" } = {}
) {
  const ai = new GoogleGenAI({ apiKey: geminiApiKey });

  const docLens = kb.chunks.map(
    (c) => (c.normText.match(/[a-z0-9]+/g) || []).length || 1
  );
  ...
  async function search(query, { k = 12, k_kw = 80, alpha = 0.55 } = {}) {
    ...
    const candidateIds = pre.length
      ? pre
      : [...Array(kb.chunks.length).keys()].slice(
          0,
          Math.min(2000, kb.chunks.length)
        );
